...
/**
 * Stable key for a response object's "example" payload. Looks at the standard
 * OAS `example` / `examples` fields and at apifier's extension shape
 * `extensions["x-response-example-<status>"]`. Returns `''` when no example is
 * present so two responses with no examples compare equal.
 * @param {object} resp
 * @returns {string}
 */
function _exampleKey(resp) {
  if (!resp || typeof resp !== 'object') return '';
  if (resp.example !== undefined)  return JSON.stringify(resp.example);
  if (resp.examples !== undefined) return JSON.stringify(resp.examples);
  return '';
}
...
function _compareResponses(aResponses, bResponses, basePath, out) {
  ...
  for (const status of Object.keys(aMap)) {
    ...
    if (!(status in bMap)) {
      ...
      const schemaChanged = _typeKey(aResp.schema) !== _typeKey(bResp.schema);
      ...
      // Example-only drift → patch. The example field is documentation, not
      // contract — a generated client behaviour is identical whether or not
      // the example string changes. Worth surfacing for audit trails.
      // Only emit when schemas matched (schema-change already implies any
      // body shift; an example-only diff under a schema change adds nothing).
      if (!schemaChanged && _exampleKey(aResp) !== _exampleKey(bResp)) {
        out.push({
          kind:   'response_example_only_changed',
          path:   `${rPath}.example`,
          impact: 'patch',
          detail: `Example body for response ${status} changed in ${basePath} (schema unchanged).`,
          before: _exampleKey(aResp),
          after:  _exampleKey(bResp),
        });
      }
