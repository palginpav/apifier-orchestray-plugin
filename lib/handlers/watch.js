...
async function handleWatch(params) {
  ...
  // Note: scrape_ms covers ONLY the network-or-file fetch + parse + write.
  // The fresh-mapping re-read (just below) is a separate I/O cost that the
  // baseline_load_ms field captures alongside the baseline read — both add
  // to total_ms but are not in scrape_ms or diff_ms.
  const scrapeMs = Number(process.hrtime.bigint() - scrapeStart) / 1e6;

  // --- Load fresh + baseline mappings (combined I/O bucket) ---
  ...
  return {
    ...
    timing: {
      ...
      // baseline_load_ms covers the fresh-mapping AND baseline-mapping disk
      // reads + zod validation. For typical sub-MB mappings this is single-
      // digit ms; for very large mappings it can be ~10s of ms. Captured
      // separately so total_ms = scrape_ms + baseline_load_ms + diff_ms
      // adds up to the actual wall-clock cost (≤ 1ms rounding gap).
      baseline_load_ms: Math.round(loadMs),
