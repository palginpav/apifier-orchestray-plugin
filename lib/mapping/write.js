...
// ---------------------------------------------------------------------------
// Allowed roots (port of arena/lib/emit.js lines 60-96).
// The caller may pass extra roots via opts.extra_allowed_roots.
// ---------------------------------------------------------------------------
...
function _buildAllowedRoots(extraRoots) {
  ...
  if (Array.isArray(extraRoots)) {
    for (const r of extraRoots) {
      // Extra roots must also pass the literal-.. check.
      if (r.includes('..')) throw new ValidatorRejectedError('extra_allowed_root contains parent traversal segment');
