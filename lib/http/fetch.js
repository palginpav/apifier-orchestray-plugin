...
const { ScrapeSizeError, AuthGatedError, RobotsBlockedError } = require('../errors');
 ...
 * @param {boolean} [opts.obey_robots_txt=true] - When true, check robots.txt before URL GETs.
 * @returns {Promise<{
 *   body: string,
 *   content_type: string,
 *   sha256: string,
 ...
 * }>}
 */
async function fetchSource({ source, timeout_ms, obey_robots_txt }) {
  ...
  // kind === 'url'
  // Robots gate — default on; skip when explicitly set to false.
  const shouldObeyRobots = obey_robots_txt !== false;
  if (shouldObeyRobots) {
    const decision = await checkRobots(source, { userAgent: USER_AGENT });
    if (!decision.allowed) {
      throw new RobotsBlockedError(
        `robots.txt blocks ${source} (rule: ${decision.matched_rule || 'disallowed'})`,
        { url: source, matched_rule: decision.matched_rule, robots_url: decision.robots_url }
      );
    }
  }
