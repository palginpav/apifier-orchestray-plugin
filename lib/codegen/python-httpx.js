...
// lib/codegen/python-httpx.js — Python httpx-client generator from an apifier-mapping v1.
// Pure-string codegen. No external deps. Byte-deterministic for identical inputs.
// Derived from python-requests.js; key differences: httpx.Client, no base64 for basic auth.
...
/**
 * Convert an enum value into UPPER_SNAKE_CASE Python identifier for enum.Enum member.
 * @param {*} value
 * @param {number} idx
 * @returns {string}
 */
...
/**
 * Sanitise a method name: convert to snake_case and suffix _op for reserved words.
 * @param {string} name
 * @returns {string}
 */
...
/**
 * Emit Python @dataclass for a single model (identical to python-requests).
 * @param {object} model
 * @returns {string}
 */
...
/**
 * Build URL expression using urllib.parse.quote for path params.
...
/**
 * Build query lines for httpx (params dict).
 ...
 * httpx uses params dict — same idiom as python-requests.
 * @param {object} endpoint
...
/**
 * Emit request headers setup lines for httpx.
 * @param {object} endpoint
 * @param {Array} authSchemes
 * @returns {string[]}
 */
function _buildHeaderLines(endpoint, authSchemes) {
  ...
  for (const schemeId of endpointAuthIds) {
    ...
    if (scheme.type === 'http-bearer') {
      ...
      // httpx supports auth tuple — no base64 needed; pass via self._client.auth
      // bearer-style header not needed; httpx will handle basic auth internally
    }
  }
...
/**
 * Collect which auth types are used.
 * @param {Array} authSchemes
...
/**
 * Emit Python auth helper methods for httpx.
 * set_basic_auth uses httpx tuple auth — no base64 dance.
...
/**
 * Build the init guide for the module docstring.
...
/**
 * Emit one client method for an endpoint (httpx version).
 * Uses self._client.request(method, url, params=..., headers=..., json=...).
 * @param {object} endpoint
 * @param {Array} authSchemes
 * @returns {string}
 */
function _emitMethod(endpoint, authSchemes) {
  ...
  // Build parameter list; track emitted names to avoid duplicates
  const sigParams = [];
  ...
  for (const qp of (endpoint.query_params || [])) {
    const id = _toIdentifier(qp.name);
    if (emittedParamNames.has(id)) continue;
    emittedParamNames.add(id);
  ...
  if (endpoint.body) {
    if (endpoint.body.content_type === 'application/json') {
      ...
      requestKwargs.push('content=body');
    }
  }
  lines.push(`        _resp = self._client.request(${requestArgs.join(', ')}, ${requestKwargs.join(', ')})`);
...
/**
 * Generate a Python httpx-based client from an apifier-mapping v1.
 *
...
function generate(mapping, opts) {
  ...
  // -------------------------------------------------------------------------
  // 2. Imports — stdlib then third-party; conditionally include `enum`
  // NOTE: `import base64` is NOT needed — httpx accepts auth=(user, pwd) tuples
  // -------------------------------------------------------------------------
  ...
  // W42-followup: `from __future__ import annotations` (PEP 563) makes ALL
  // dataclass field annotations lazy strings, so a model that forward-
  // references another model defined later in the file (we sort models
  // alphabetically — Pet ships before Tag) doesn't trip a NameError at
  // class-definition time. Python 3.7+; we target 3.8+, safe.
  parts.push([
  ...
  // -------------------------------------------------------------------------
  // 4. Error class hierarchy (identical to python-requests)
  // -------------------------------------------------------------------------
  parts.push([
  ...
  // Constructor — creates httpx.Client with base_url and timeout
  classLines.push(`    def __init__(self, base_url: Optional[str] = None) -> None:`);
  classLines.push(`        _base_url: str = base_url if base_url is not None else ${JSON.stringify(baseUrl)}`);
  classLines.push('        # Remove trailing slash for consistent URL construction');
  classLines.push("        if _base_url.endswith('/'):");
  classLines.push("            _base_url = _base_url[:-1]");
  classLines.push('        self._client: httpx.Client = httpx.Client(base_url=_base_url, timeout=30.0)');
  // W42 fix: per-method URL expressions reference self._base_url; assign it
  // here so f"{self._base_url}/..." resolves at runtime. Without this, every
  // method call would raise AttributeError (py_compile catches syntax only).
  classLines.push('        self._base_url: str = _base_url');
...
module.exports = { generate, PARSER_NAME: 'apifier-python-httpx-generator', PARSER_VERSION: '0.0.1' };
