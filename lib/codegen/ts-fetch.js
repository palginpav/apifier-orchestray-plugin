 ...
 * @returns {string[]} Lines of code.
 */
function _buildResponseLines(endpoint) {
  const lines = [];
  const responses = endpoint.responses || {};
  ...
  // Generic fallback
  lines.push('  if (_res.status >= 200 && _res.status < 300) return _res.json() as Promise<ReturnType<typeof this.' + _toMethodName(endpoint.id) + '>>;');
...
function _emitMethod(endpoint, authSchemes) {
  ...
  // Response handling
  const respLines = _buildResponseLines(endpoint);
