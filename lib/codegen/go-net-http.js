...
/**
 * Build the URL expression with path parameter substitution.
 * Returns Go code lines for building the request URL.
 * @param {string} urlPath - The endpoint path template.
 * @param {Array} pathParams - The path_params array.
 * @param {string} baseUrlVar - The base URL variable name.
 * @returns {string[]} Lines of Go code.
 */
function _buildUrlLines(urlPath, pathParams, baseUrlVar) {
  const lines = [];
  const paramNames = new Set((pathParams || []).map(p => p.name));

  // Build the path string with url.PathEscape substitutions
  // Replace {param} → " + url.PathEscape(args.Param) + "
  let pathExpr = urlPath;
  for (const paramName of paramNames) {
    const fieldName = _toIdentifier(paramName);
    const exportedField = fieldName.charAt(0).toUpperCase() + fieldName.slice(1);
    // Replace {paramName} placeholder
    pathExpr = pathExpr.replace(
      new RegExp(`\\{${paramName}\\}`, 'g'),
      `" + url.PathEscape(fmt.Sprintf("%v", args.${exportedField})) + "`
    );
  }

  lines.push(`\t_rawURL := ${JSON.stringify(baseUrlVar)} + ${JSON.stringify(pathExpr)}`);
  // Clean up concatenation artifacts: "" + x + "" → x
  // We'll use a more direct approach — build piecemeal
  return _buildUrlLinesDirect(urlPath, pathParams, baseUrlVar);
}
...
function generate(mapping, opts) {
  ...
  // errors always needed for ErrApifierClientErr
  neededImports.add('errors');
