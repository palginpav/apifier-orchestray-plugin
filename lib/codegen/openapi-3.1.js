...
/**
 * Emit a YAML block-style list.
 * Each element is formatted with an indent-level prefix.
 *
 * @param {Array} items   - Array of pre-formatted YAML strings (each item body).
 * @param {number} indent - Number of spaces for the list items' content indent.
 * @returns {string[]}    - Lines (without trailing newline per line).
 */
function _yamlList(items, indent) {
  const indentStr = ' '.repeat(indent);
  const lines = [];
  for (const item of items) {
    // item is a pre-rendered string; its first line gets the "- " prefix
    const itemLines = item.split('\n');
    lines.push(`${indentStr}- ${itemLines[0]}`);
    for (let i = 1; i < itemLines.length; i++) {
      // continuation lines get extra indentation (aligned with after the "- ")
      lines.push(`${indentStr}  ${itemLines[i]}`);
    }
  }
  return lines;
}
...
/**
 * Emit a YAML block-style mapping.
 * Keys are emitted in the order given by `keyOrder`.
 *
 * @param {object} obj       - The object to emit.
 * @param {string[]} keyOrder - Explicit key order; keys absent from obj are skipped.
 * @param {number} indent    - Number of spaces for this level's content.
 * @returns {string[]}       - Lines (no trailing newline per line).
 */
function _yamlObject(obj, keyOrder, indent) {
  const indentStr = ' '.repeat(indent);
  const lines = [];
  ...
  for (const key of keyOrder) {
    ...
    const val = obj[key];
    const keyStr = _yamlScalar(key);

    if (val === null) {
      lines.push(`${indentStr}${keyStr}: null`);
    } else if (typeof val === 'boolean') {
      lines.push(`${indentStr}${keyStr}: ${val ? 'true' : 'false'}`);
    } else if (typeof val === 'number') {
      lines.push(`${indentStr}${keyStr}: ${val}`);
    } else if (typeof val === 'string') {
      lines.push(`${indentStr}${keyStr}: ${_yamlScalar(val)}`);
    } else if (Array.isArray(val)) {
      if (val.length === 0) {
        lines.push(`${indentStr}${keyStr}: []`);
      } else {
        lines.push(`${indentStr}${keyStr}:`);
        // Each element: if string/scalar, emit as list; if object, recurse
        for (const item of val) {
          if (item === null || typeof item !== 'object') {
            lines.push(`${indentStr}  - ${_yamlScalar(item)}`);
          } else {
            // Object item: emit first key on same line as "- ", rest indented
            const itemKeyOrder = Object.keys(item);
            const itemLines = _yamlObjectInline(item, itemKeyOrder, indent + 2);
            if (itemLines.length === 0) {
              lines.push(`${indentStr}  - {}`);
            } else {
              lines.push(`${indentStr}  - ${itemLines[0].trimStart()}`);
              for (let i = 1; i < itemLines.length; i++) {
                lines.push(`${indentStr}    ${itemLines[i].trimStart()}`);
              }
            }
          }
        }
      ...
      if (keys.length === 0) {
        ...
        lines.push(`${indentStr}${keyStr}:`);
        // Recurse: use explicit key order if it's a typed sub-object, else natural order
        const subKeyOrder = val.__keyOrder || Object.keys(val);
        for (const subLine of _yamlObjectInline(val, subKeyOrder, indent + 2)) {
          lines.push(subLine);
        }
      }
    }
...
/**
 * Internal helper: emit an object's key-value pairs as indented lines.
 * Unlike _yamlObject, this does NOT add an outer indent — lines already carry it.
 *
 * @param {object} obj
 * @param {string[]} keyOrder
 * @param {number} indent - absolute indent for these lines
 * @returns {string[]}
 */
function _yamlObjectInline(obj, keyOrder, indent) {
  const indentStr = ' '.repeat(indent);
  const lines = [];

  for (const key of keyOrder) {
    if (key === '__keyOrder') continue;
    if (!(key in obj) || obj[key] === undefined) continue;
    const val = obj[key];
    const keyStr = _yamlScalar(key);
    ...
    if (val === null) {
      ...
      if (val.length === 0) {
        ...
        for (const item of val) {
          if (item === null || typeof item !== 'object') {
            ...
            const itemKeyOrder = item.__keyOrder || Object.keys(item).filter(k => k !== '__keyOrder');
            const itemLines = _yamlObjectInline(item, itemKeyOrder, indent + 4);
            if (itemLines.length === 0) {
              lines.push(`${indentStr}  - {}`);
            } else {
              lines.push(`${indentStr}  - ${itemLines[0].slice(indent + 4)}`);
              for (let i = 1; i < itemLines.length; i++) {
                lines.push(`${indentStr}    ${itemLines[i].slice(indent + 4)}`);
              }
            }
          }
        }
      ...
      const keys = Object.keys(val).filter(k => k !== '__keyOrder');
      if (keys.length === 0) {
        lines.push(`${indentStr}${keyStr}: {}`);
      } else {
        lines.push(`${indentStr}${keyStr}:`);
        const subKeyOrder = val.__keyOrder || keys;
        for (const subLine of _yamlObjectInline(val, subKeyOrder.filter(k => k !== '__keyOrder'), indent + 2)) {
          lines.push(subLine);
        }
      }
    }
...
/**
 * Emit the full YAML document from a root object.
 * The root always starts at indent 0.
 *
 * @param {object} rootObj - Object with __keyOrder for deterministic emission.
 * @returns {string}  Complete YAML document string ending with '\n'.
 */
function _emitYaml(rootObj) {
  const keyOrder = rootObj.__keyOrder || Object.keys(rootObj).filter(k => k !== '__keyOrder');
  const lines = _yamlObjectInline(rootObj, keyOrder, 0);
  return lines.join('\n') + '\n';
}
...
// ---------------------------------------------------------------------------
// Deterministic YAML serialiser for nested structures
// ---------------------------------------------------------------------------

/**
 * Recursively render an object into flat YAML lines with explicit key ordering.
 * This is the unified emitter used for the full document tree.
 *
 * @param {*} value         - The value to render.
 * @param {number} indent   - Current indentation level (spaces).
 * @returns {string[]}      - Lines (already indented).
 */
function _renderValue(value, indent) {
  const pad = ' '.repeat(indent);

  if (value === null || value === undefined) return [`${pad}null`];
  if (typeof value === 'boolean') return [`${pad}${value ? 'true' : 'false'}`];
  if (typeof value === 'number') return [`${pad}${value}`];
  if (typeof value === 'string') return [`${pad}${_yamlScalar(value)}`];
  ...
  if (Array.isArray(value)) {
    ...
    for (const item of value) {
      if (item === null || typeof item !== 'object') {
        ...
        const itemLines = _renderObject(item, indent + 2);
        if (itemLines.length === 0) {
          lines.push(`${pad}- {}`);
        } else {
          // First line loses indent and gets "- " prefix
          lines.push(`${pad}- ${itemLines[0].slice(indent + 2)}`);
          for (let i = 1; i < itemLines.length; i++) {
            lines.push(`${pad}  ${itemLines[i].slice(indent + 2)}`);
          }
        }
      }
    }
  ...
  if (typeof value === 'object') {
    return _renderObject(value, indent);
  }

  return [`${pad}${_yamlScalar(String(value))}`];
}

/**
 * Render an object's key-value pairs as YAML lines.
 * If obj has __keyOrder, use that order; otherwise natural insertion order.
 *
 * @param {object} obj
 * @param {number} indent
 * @returns {string[]}
 */
function _renderObject(obj, indent) {
  const pad = ' '.repeat(indent);
  const keyOrder = obj.__keyOrder
    ? obj.__keyOrder.filter(k => k !== '__keyOrder' && k in obj && obj[k] !== undefined)
    : Object.keys(obj).filter(k => k !== '__keyOrder' && obj[k] !== undefined);

  if (keyOrder.length === 0) return [`${pad}{}`];
  ...
  for (const key of keyOrder) {
    ...
    if (val === null) {
      ...
      lines.push(`${pad}${keyStr}: ${val ? 'true' : 'false'}`);
    } else if (typeof val === 'number') {
      lines.push(`${pad}${keyStr}: ${val}`);
    } else if (typeof val === 'string') {
      lines.push(`${pad}${keyStr}: ${_yamlScalar(val)}`);
    } else if (Array.isArray(val)) {
      if (val.length === 0) {
        lines.push(`${pad}${keyStr}: []`);
      } else {
        lines.push(`${pad}${keyStr}:`);
        const itemLines = _renderValue(val, indent + 2);
        for (const l of itemLines) lines.push(l);
      }
    } else if (typeof val === 'object') {
      const subKeys = val.__keyOrder
        ? val.__keyOrder.filter(k => k !== '__keyOrder' && k in val && val[k] !== undefined)
        : Object.keys(val).filter(k => k !== '__keyOrder' && val[k] !== undefined);

      if (subKeys.length === 0) {
        lines.push(`${pad}${keyStr}: {}`);
      } else {
        lines.push(`${pad}${keyStr}:`);
        const subLines = _renderObject(val, indent + 2);
