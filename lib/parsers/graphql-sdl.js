...
function _tokeniseDeclarations(stripped) {
  ...
  while (i < len) {
    ...
    // Skip whitespace / implements clause. STOP at '=' too so that union
    // declarations (`union X = A | B | C`) preserve their body for the
    // union-specific parser below — without this stop, the implements-skip
    // loop consumes the union's member list and unionBody ends up empty.
    while (
      i < len &&
      stripped[i] !== '{'  &&
      stripped[i] !== '\n' &&
