...
// lib/parsers/openapi.js — OpenAPI 3.0/3.1 (and best-effort Swagger 2.0) parser → IR. JSON + YAML supported.

const jsYaml = require('js-yaml');
const { OpenAPIParseError } = require('../errors');
...
/**
 * Detect whether body/contentType/source_url indicates YAML format.
 *
 * Triggers YAML parsing when ANY of:
 *   1. Content-Type carries `yaml` (case-insensitive)
 *   2. sourceUrl ends in `.yaml` or `.yml`
 *   3. Body has no leading `{` AND starts with `openapi:`, `swagger:`, or a
 *      YAML doc marker (`---`)
 *
 * Known gap: a valid YAML OpenAPI body whose FIRST key is something other
 * than `openapi:`/`swagger:` (e.g. `info:`) AND that has no content-type or
 * `.yaml` URL hint falls through to JSON.parse and surfaces as
 * `OpenAPIParseError`. Conformant specs always start with `openapi:` /
 * `swagger:`, so this is acceptable for v0.0.1.
 *
 * @param {string} body
 ...
 * @returns {boolean}
 */
function _isYaml(body, contentType, sourceUrl) {
  if (/yaml/i.test(contentType)) return true;
  if (sourceUrl && /\.ya?ml$/i.test(sourceUrl)) return true;
  const trimmed = body.trimStart();
  if (trimmed.startsWith('{')) return false;
  if (/^openapi\s*:|^swagger\s*:|^---\s*\n|^---\s*$/.test(trimmed)) return true;
  return false;
}

/**
 * Parse raw spec string as JSON or YAML and return parsed object.
 * YAML uses JSON_SCHEMA to reject unsafe tags (!!js/function, !!python/*, etc.).
 * @param {string} rawBody
 * @param {string} contentType
 * @param {string|null} sourceUrl
 * @returns {object} Parsed document
 */
function _parseDocBody(rawBody, contentType, sourceUrl) {
  if (_isYaml(rawBody, contentType, sourceUrl)) {
    try {
      return jsYaml.load(rawBody, { schema: jsYaml.JSON_SCHEMA, json: false });
    } catch (err) {
      throw Object.assign(
        new OpenAPIParseError(`Failed to parse spec as YAML: ${err.message}`),
        { data: { format: 'yaml' } }
      );
    }
  }
  ...
  try {
    ...
    throw new OpenAPIParseError(`Failed to parse spec as JSON: ${err.message}`);
  }
}
...
async function parseOpenAPI({ body, content_type, source_url }) {
  ...
  const doc = _parseDocBody(body, content_type || '', source_url || null);
