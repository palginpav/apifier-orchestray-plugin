...
function _deriveId(name, method, usedIds) {
  // Lowercase the name slug for cross-parser consistency: openapi.js emits
  // lowercase IDs, so the postman parser does the same to keep downstream
  // tooling (mapping comparison, codegen name resolution) uniform.
  const nameSlug = name.replace(/[^A-Za-z0-9]+/g, '_').replace(/^_+|_+$/g, '').toLowerCase();
 ...
 */
// Cap recursion depth defensively (Postman folder nesting >100 is pathological
// — real-world collections rarely exceed 5). Prevents stack overflow on
// adversarial or corrupted collection files.
const MAX_FOLDER_DEPTH = 100;

function _walkItems(items, folderPath, out, counters, depth) {
  if (!Array.isArray(items)) return;
  const currentDepth = depth || 0;
  if (currentDepth > MAX_FOLDER_DEPTH) {
    throw new PostmanParseError(
      `Postman collection folder nesting exceeds maximum depth of ${MAX_FOLDER_DEPTH} ` +
      `(adversarial or corrupted collection?)`,
    );
  }
  for (const item of items) {
    ...
    if (Array.isArray(item.item)) {
      ...
      _walkItems(item.item, [...folderPath, item.name || ''], out, counters, currentDepth + 1);
...
async function parsePostman({ body, content_type, source_url }) {
  ...
  for (const { item, folderPath } of leafItems) {
    ...
    // Parse URL with variable substitution. The `{{var}}` placeholders may
    // appear in either a string-form `req.url` OR in `req.url.raw` / host
    // segments of a structured URL object. Substitute in both shapes so the
    // common `{{baseUrl}}` Postman pattern resolves correctly (otherwise
    // _extractOrigin returns the unresolved `https://{baseurl}` literal).
    let rawUrl;
    if (typeof req.url === 'string') {
      rawUrl = _resolveVars(req.url, varMap);
    } else if (req.url && typeof req.url === 'object') {
      // Clone-and-substitute the fields _parseUrl reads from.
      const u = req.url;
      rawUrl = {
        ...u,
        raw:      typeof u.raw  === 'string' ? _resolveVars(u.raw, varMap)  : u.raw,
        host:     Array.isArray(u.host)
                    ? u.host.map(h => typeof h === 'string' ? _resolveVars(h, varMap) : h)
                    : (typeof u.host === 'string' ? _resolveVars(u.host, varMap) : u.host),
        path:     Array.isArray(u.path)
                    ? u.path.map(p => typeof p === 'string' ? _resolveVars(p, varMap) : p)
                    : u.path,
        protocol: typeof u.protocol === 'string' ? _resolveVars(u.protocol, varMap) : u.protocol,
      };
    } else {
      rawUrl = req.url;
    }
    ...
    const allQuery = (rawUrl && typeof rawUrl === 'object' && Array.isArray(rawUrl.query))
      ? rawUrl.query
      : [];
