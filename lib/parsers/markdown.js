...
function _parseEndpointSection(endpoint, sectionLines, allBlocks, sectionLineOffset, warnings) {
  ...
  for (const block of blocksInSection) {
    ...
    // Identify which sub-section this block falls under AND capture the
    // heading line that owns it. The heading text is used for per-response
    // status extraction so multi-response endpoints don't collapse all
    // their bodies under whichever heading was last seen during the
    // forward line-scan.
    let blockSubsection = null;
    let blockHeadingLine = '';
    for (let k = relLine - 1; k >= 0; k--) {
      if (/^#{3,4}\s/.test(sectionLines[k])) {
        blockHeadingLine = sectionLines[k];
    ...
    } else if (blockSubsection === 'response') {
      // Status code comes from THIS block's nearest preceding heading line,
      // not the shared `responseHeadingText` which only holds the last one
      // seen during the forward scan. Otherwise multi-response endpoints
      // would collapse every body under the final heading's status.
      let status = '200';
      const statusMatch = blockHeadingLine.match(/(\d{3})/);
    ...
    } else if (blockSubsection === null && isJson && responseHeadingText) {
      // Tightened fallback (W24-3): only treat a heading-less JSON block as
      // a response if the section actually had at least one response heading
      // somewhere. Prevents intro-paragraph JSON from being silently mis-
      // classified as a response example.
      let status = '200';
...
async function parseMarkdown({ body, content_type, source_url }) {
  ...
  if (serviceDisplayName) {
    ...
    try {
      ...
      const base = path.basename(source_url, '.md').replace(/[^a-z0-9]+/gi, '-').toLowerCase();
